...
def global_init(x, time_and_machindex):
    ...
    while True:
        ...
        while True:
            ...
            cur = time_and_machindex[
                job_list[job_index] * process + process_list[process_index]
            ]
            ...
            min_load_list = [
                i + 1 for i in range(len(temp_machload)) if temp_machload[i] == min_load
            ]
            min_time = np.inf
            ...
            x[
                total_process
                + job_list[job_index] * process
                + process_list[process_index]
            ] = min_time_list[np.random.randint(0, len(min_time_list))]
            machload[
                int(
                    x[
                        total_process
...
def local_init(x, time_and_machindex):
    ...
    for p in range(workpiece):
        ...
        while True:
            ...
            min_time_list = [i[1] for i in cur if i[0] == min_time]
            x[
                total_process + p * process + process_list[process_index]
            ] = min_time_list[np.random.randint(0, len(min_time_list))]
            machload[
                int(x[total_process + p * process + process_list[process_index]] - 1)
            ] += min_time
            del process_list[process_index]
        cur_begin += process
